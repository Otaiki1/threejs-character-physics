...
import { GUI } from "../components/threejs/GUI";
import { ModelLoader } from "../components/threejs/ModelLoader";
import { TextManager } from "../components/threejs/TextManager";
...
export const useGame = (
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (gameRefs.current.character && gameRefs.current.characterBody) {
        ...
        // Handle animations
        if (gameRefs.current.mixer && gameRefs.current.animationsMap) {
          handleCharacterAnimations(
      ...
      if (gameRefs.current.character && gameRefs.current.camera) {
        updateCamera(gameRefs.current.character, gameRefs.current.camera);
      }
    ...
    const onWindowResize = () => {
      if (gameRefs.current.camera) {
        gameRefs.current.camera.aspect = window.innerWidth / window.innerHeight;
